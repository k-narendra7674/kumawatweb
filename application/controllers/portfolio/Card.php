...
class Card extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('portfolio/Card_Model');
	}

	public function index()
	{
		$this->load->view('admin/header');
		$this->load->view('project/portfolio/card_list');
	...
	public function addCard()
	{
		...
		// 	// $this->form_validation->set_rules('content', 'Content', 'required');
			...
			if ($this->form_validation->run() ==  FALSE) {
				...
				$config['upload_path'] = APPPATH . '../public/uploads/cards/';
				...
				if (!$this->upload->do_upload('img')) {
					...
					if ($this->Card_Model->create($ajax_data)) {
	...
	public function delCard()
	{
		if ($this->input->is_ajax_request()) {
			...
			$post = $this->Card_Model->single_id($del_id);
			unlink(APPPATH . '../public/uploads/cards/' . $post->img);

			if ($this->Card_Model->delete($del_id)) {
	...
	public function editCard()
	{
		if ($this->input->is_ajax_request()) {
			...
			if ($post = $this->Card_Model->single_id($edit_id)) {
		...
		if ($this->input->is_ajax_request()) {
			...
			if ($this->form_validation->run() ==  FALSE) {
				...
				if (isset($_FILES['edit_card_img'])) {
					...
					if (!$this->upload->do_upload('edit_card_img')) {
						$data = array('result' => 'error', 'message' => $this->upload->display_errors());
					} else {
				...
				if ($this->Card_Model->update($id, $ajax_data)) {
