...
class App extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('portfolio/App_Model');
	}

	public function index()
	{
		$this->load->view('admin/header');
		$this->load->view('project/portfolio/app_list');
	...
	public function addApp()
	{
		...
		// 	// $this->form_validation->set_rules('content', 'Content', 'required');
			...
			if ($this->form_validation->run() ==  FALSE) {
				...
				$config['upload_path'] = APPPATH . '../public/uploads/apps/';
				...
				if (!$this->upload->do_upload('img')) {
					...
					if ($this->App_Model->create($ajax_data)) {
	...
	public function delApp()
	{
		if ($this->input->is_ajax_request()) {
			...
			$post = $this->App_Model->single_id($del_id);
			unlink(APPPATH . '../public/uploads/apps/' . $post->img);

			if ($this->App_Model->delete($del_id)) {
	...
	public function editApp()
	{
		if ($this->input->is_ajax_request()) {
			...
			if ($post = $this->App_Model->single_id($edit_id)) {
		...
		if ($this->input->is_ajax_request()) {
			...
			if ($this->form_validation->run() ==  FALSE) {
				...
				if (isset($_FILES['edit_app_img'])) {
					...
					if (!$this->upload->do_upload('edit_app_img')) {
						$data = array('result' => 'error', 'message' => $this->upload->display_errors());
					} else {
				...
				if ($this->App_Model->update($id, $ajax_data)) {
