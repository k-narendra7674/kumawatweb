...
class About extends CI_Controller
{
		...
		if ($this->input->is_ajax_request()) {
			...
			if ($this->form_validation->run() ==  FALSE) {
				...
				if (!$this->upload->do_upload('img')) {
					...
					if ($this->About_model->create($ajax_data)) {
	...
	public function delAbout()
	{
		if ($this->input->is_ajax_request()) {
			...
			$post = $this->About_model->single_id($del_id);
			unlink(APPPATH . '../public/uploads/' . $post->img);

			if ($this->About_model->delete($del_id)) {
	...
	public function editAbout()
	{
		if ($this->input->is_ajax_request()) {
			...
			if ($post = $this->About_model->single_id($edit_id)) {
	...
	public function updateAbout()
	{
		if ($this->input->is_ajax_request()) {
			...
			if ($this->form_validation->run() ==  FALSE) {
				...
				if (isset($_FILES['edit_about_img'])) {
					$config['upload_path'] = APPPATH . '../public/uploads/';
					$config['allowed_types']        = 'gif|jpg|png';
					$config['max_size']             = '1000';
					//              // $config['max_width']            = 1024;
					...
					if (!$this->upload->do_upload('edit_about_img')) {
						$data = array('result' => 'error', 'message' => $this->upload->display_errors());
					} else {
				...
				if ($this->About_model->update($id, $ajax_data)) {
